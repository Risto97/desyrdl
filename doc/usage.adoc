...
```
The `RdlFormatter` class implements a custom language for the format specifier that gives access to the compiled SystemRDL tree.
...
* 'repeat:rtype:string': the loop functionality of `RdlFormatter`. Value must be a Python dict (the "context") that must contain the key given by `rtype` (except 'fields', where `value` is a `RegNode`).
** `rtype==regtypes`
** `rtype==regnames`
...
All RDL files must be passed to RdlFormatter.py in the correct order.
